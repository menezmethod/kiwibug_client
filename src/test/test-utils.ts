...
import { projectGenerator, userGenerator } from './data-generators';
...
export const createProject = async (projectProperties?: any) => {
  const project = projectGenerator(projectProperties);
  const res = await db.discussion.create(project);
  return res;
};
